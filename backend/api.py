"""
WTI Crude Oil — FastAPI Backend (TensorFlow)
============================================
Loads pre-trained model weights at startup.
...
MC_PATH     = MODELS_DIR / "mc_cnn_best.keras"
...
# ─── Custom layer — must match EXACTLY how it was saved ───────────────────────
# The saved config uses keys: window, d_model  (from the training script)
# So get_config() must return those same keys.

@tf.keras.utils.register_keras_serializable(package="Custom")
class PositionalEncoding(layers.Layer):
    """
    Matches the PositionalEncoding class used during training.
    Config keys: window, d_model  (as saved in cbam_cnn_fixed.keras)
    """
...
# ─── GCN builder (weights-only load, no .keras) ───────────────────────────────
...
def build_gcn(window, num_feat):
    ...
    x   = layers.Activation("relu")(bn1(gc1(inp, A_n))); x = dr(x)
    x   = layers.Activation("relu")(bn2(gc2(x,   A_n))); x = dr(x)
    x   = layers.Activation("relu")(bn3(gc3(x,   A_n)))
...
async def startup():
    missing = [str(p) for p in [CBAM_PATH, CBAM_STATS, MC_PATH, MC_STATS,
                                  GCN_WEIGHTS, GCN_ADJ, GCN_STATS]
               if not p.exists()]
    if missing:
...
def predict():
    ...
    return {
        ...
        "predictions":   {"CBAM_CNN": round(cbam_pred, 2), "MC_CNN": round(mc_pred, 2),
                          "GCN": round(gcn_pred, 2), "ensemble": round(ensemble, 2)},
        "deltas":        {"CBAM_CNN": round(cbam_pred-last_price, 2),
...
def forecast(days: int = 5):
    ...
    for i in range(days):
        ...
        last_p = raw_prices[-1]
        result.append({"day": i+1, "CBAM_CNN": round(cbam_p, 2), "MC_CNN": round(mc_p, 2),
                        "GCN": round(gcn_p, 2), "ensemble": round(ens_p, 2),
    ...
    return {"current_price": round(STATE["raw_prices"][-1], 2),
            "forecast": result, "timestamp": datetime.now().isoformat()}
